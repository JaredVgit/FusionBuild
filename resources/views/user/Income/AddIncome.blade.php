...
@section('title', 'Add Income | FusionBuild')

@section('content')
<div class="flex justify-center items-center min-h-screen">
    <div class="bg-grayLight p-6 md:p-8 rounded-lg shadow-lg w-full sm:w-3/4 lg:w-1/2">
        <h2 class="text-2xl font-bold text-primary mb-6 text-black">Add Income</h2>
        ...
        <form action="{{ route('storeIncome') }}" method="POST" class="space-y-4">
            @csrf
            <div class="space-y-2">
...
<div class="space-y-2">
    ...
    <select name="mode_of_payment" id="mode_of_payment" class="w-full p-3 border border-gray-300 rounded-lg text-black focus:ring focus:ring-blue-300" required>
        <option value="" disabled {{ old('mode_of_payment', $transaction->mode_of_payment ?? '') == '' ? 'selected' : '' }}>Select Payment Mode</option>
        <option value="cash" {{ old('mode_of_payment', $transaction->mode_of_payment ?? '') == 'cash' ? 'selected' : '' }}>Cash</option>
        <option value="bank transfer" {{ old('mode_of_payment', $transaction->mode_of_payment ?? '') == 'bank transfer' ? 'selected' : '' }}>Bank Transfer</option>
        <option value="gcash" {{ old('mode_of_payment', $transaction->mode_of_payment ?? '') == 'gcash' ? 'selected' : '' }}>GCash</option>
        <option value="check" {{ old('mode_of_payment', $transaction->mode_of_payment ?? '') == 'check' ? 'selected' : '' }}>Check</option>
    </select>
</div>
    ...
    onclick="return confirm('Are you sure you want to add this income record?');">
    Add Income
</button>
